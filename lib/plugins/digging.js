...
function digActionStatus (bot, action) {
  if (bot.version === '26.3') {
    return { start: 0, cancel: 2, finish: 3 }[action]
  }
  return { start: 0, cancel: 1, finish: 2 }[action]
}
...
function inject (bot) {
  ...
  async function dig (block, forceLook, digFace) {
    ...
    bot._client.write('block_dig', {
      status: digActionStatus(bot, 'start'),
    ...
    function finishDigging () {
      ...
      if (bot.targetDigBlock) {
        bot._client.write('block_dig', {
          status: digActionStatus(bot, 'finish'),
    ...
    bot.stopDigging = () => {
      ...
      waitTimeout = null
      bot._client.write('block_dig', {
        status: digActionStatus(bot, 'cancel'),
        location: bot.targetDigBlock.position,
