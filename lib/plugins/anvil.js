...
const { once, onceWithCleanup } = require('../promise_utils')
  ...
  async function openAnvil (anvilBlock) {
    ...
    async function addCustomName (name) {
      ...
      for (let i = 1; i < name.length + 1; i++) {
        const resultUpdate = onceWithCleanup(anvil, `updateSlot:${anvil.craftingResultSlot}`, {
          timeout: 5000,
          checkCondition: (_oldItem, newItem) => Boolean(newItem)
        })
        sendItemName(name.substring(0, i))
        // The result slot is server-computed. Waiting for it avoids picking
        // up the previous character when 26.2 processes name_item and the
        // following window click in different ticks.
        await resultUpdate
      }
    }
    async function putInAnvil (itemOne, itemTwo) {
      await putSomething(0, itemOne.type, itemOne.metadata, itemOne.count, itemOne.nbt, itemOne)
      sendItemName('') // sent like this by vnailla
      if (!bot.supportFeature('useMCItemName')) sendItemName('')
      await putSomething(1, itemTwo.type, itemTwo.metadata, itemTwo.count, itemTwo.nbt, itemTwo)
    }
    ...
    async function rename (item, name) {
      ...
      await putSomething(0, item.type, item.metadata, item.count, item.nbt, item)
      sendItemName('') // sent like this by vnailla
      if (!bot.supportFeature('useMCItemName')) sendItemName('')
      await addCustomName(name)
      await bot.putAway(2)
    ...
    async function putSomething (destSlot, itemId, metadata, count, nbt, sourceItem) {
      const options = {
        window: anvil,
        itemType: itemId,
        metadata,
