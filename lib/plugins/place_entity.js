...
function inject (bot) {
  ...
  async function placeEntityWithOptions (referenceBlock, faceVector, options) {
    ...
    } else if (name.endsWith('_boat') && !bot.registry.entitiesByName[name]) {
      // 1.21+ registers each wood variant as its own entity; older versions
      // expose only the generic Boat entity.
      name = 'boat'
    }
  ...
  function waitForEntitySpawn (name, placePosition) {
    const maxDistance = name === 'bat' || name === 'boat' || name.endsWith('_boat') ? 3 : 2
    let mobName = name
    if (name === 'end_crystal') {
