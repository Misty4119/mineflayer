...
function inject (bot, { hideErrors }) {
  ...
  async function waitForSlotUpdateOrWindowSync (window, slot) {
    if (slot < 0) return
    const abort = new AbortController()
    const slotUpdate = onceWithCleanup(window, `updateSlot:${slot}`, {
      timeout: WINDOW_TIMEOUT,
      signal: abort.signal
    })
    const cursorUpdate = onceWithCleanup(window, 'setCursorItem', {
      timeout: WINDOW_TIMEOUT,
      signal: abort.signal
    })
    const windowUpdate = onceWithCleanup(bot, `setWindowItems:${window.id}`, {
      timeout: WINDOW_TIMEOUT,
      signal: abort.signal
    })
    try {
      await Promise.race([slotUpdate, cursorUpdate, windowUpdate])
    } catch (err) {
      // Some 26.2 inventory clicks are accepted without a per-slot response.
      // Force one authoritative full-window response instead of waiting on an
      // event that the server is allowed to omit.
      await syncWindow(window)
    } finally {
      abort.abort()
    }
  }

  async function waitForAnvilResultUpdate (window, slot) {
    const abort = new AbortController()
    const cursorUpdate = onceWithCleanup(window, 'setCursorItem', {
      timeout: WINDOW_TIMEOUT,
      signal: abort.signal
    ...
    try {
      await Promise.race([cursorUpdate, slotUpdate, windowUpdate])
    } catch (err) {
      // The result slot is not always echoed after an anvil pickup. A full
      // sync is the bounded fallback when the server omits all three signals.
      await syncWindow(window)
    } finally {
      abort.abort()
    }
  ...
  async function waitForAnvilResultAvailable (window, slot) {
    if (window.slots[slot]) return
    const resultUpdate = onceWithCleanup(window, `updateSlot:${slot}`, {
      timeout: WINDOW_TIMEOUT,
      checkCondition: (_oldItem, newItem) => Boolean(newItem)
    })
    try {
      await resultUpdate
    } catch (err) {
      // A full sync is the bounded fallback when the server omitted the
      // result-slot packet. Do not accept an unrelated cursor update here:
      // putAway must never click an empty anvil result slot.
      await syncWindow(window)
      if (!window.slots[slot]) throw err
    }
  }

  async function waitForClickUpdate (window, slot, selectedItemBeforeClick) {
    // A pickup click is locally predicted by prismarine-windows, but modern
    // servers do not necessarily emit a packet for the initial cursor pickup.
    // The next click that consumes that prediction must still wait for its
    // target slot (or a full sync). Anvil result pickups additionally accept a
    // cursor update because 26.2 does not always echo the result slot.
    if (window.type !== 'minecraft:anvil') return false
    if (!selectedItemBeforeClick && slot === window.craftingResultSlot) {
      await waitForAnvilResultUpdate(window, slot)
      return true
    }
    // A transfer from the player inventory starts with a click on a slot that
    // contains the item while the cursor is empty.  26.2 acknowledges that
    // pickup with set_cursor_item, and the next input-slot click must wait for
    // it or it reuses the stale stateId. Empty-slot clicks still need no wait.
    if (!selectedItemBeforeClick) return false
    await waitForSlotUpdateOrWindowSync(window, slot)
    return true
  }
  ...
  async function putSelectedItemRange (start, end, window, slot) {
    ...
    let moves = 0
    const maxMoves = Math.max(1, end - start + 2)
    ...
    while (window.selectedItem) {
      if (++moves > maxMoves) {
        throw new Error(`Unable to return selected item to inventory after ${maxMoves} moves`)
      }
  ...
  async function activateEntity (entity) {
    ...
    writeUseEntity(entity, new Vec3(0, 0, 0))
  }

  async function activateEntityAt (entity, position) {
    // TODO: tell the server that we are not sneaking while doing this
    await bot.lookAt(position, false)
    writeUseEntity(entity, position.subtract(entity.position))
  }

  function writeUseEntity (entity, location) {
    ...
    bot._client.write('use_entity', {
      target: entity.id,
      mouse: location.x === 0 && location.y === 0 && location.z === 0 ? 0 : 2,
      sneaking: false,
      hand: 0,
      x: location.x,
      y: location.y,
  ...
  async function transfer (options) {
    ...
    async function transferOne () {
      ...
      if (!window.selectedItem || window.selectedItem.type !== itemType ||
        ...
        const sourceItem = findTransferSource(window, sourceStart, sourceEnd, itemType, metadata, nbt, options.sourceItem)
  ...
  function findTransferSource (window, start, end, itemType, metadata, nbt, wantedItem) {
    if (!wantedItem) return window.findItemRange(start, end, itemType, metadata, false, nbt)
    for (let i = start; i < end; i++) {
      const item = window.slots[i]
      if (item && item.type === itemType && item.metadata === metadata && sameTransferItem(item, wantedItem)) return item
    }
    return null
  }

  function sameTransferItem (item, wantedItem) {
    const withoutDefaultDamage = components => (components || []).filter(component => !(component.type === 'damage' && component.data === 0))
    return item.type === wantedItem.type &&
      item.metadata === wantedItem.metadata &&
      JSON.stringify(item.nbt) === JSON.stringify(wantedItem.nbt) &&
      JSON.stringify(withoutDefaultDamage(item.components)) === JSON.stringify(withoutDefaultDamage(wantedItem.components)) &&
      JSON.stringify(item.removedComponents || []) === JSON.stringify(wantedItem.removedComponents || [])
  }
  ...
  async function clickWindow (slot, mouseButton, mode) {
    ...
    const selectedItemBeforeClick = window.selectedItem
    const syncAfterPickup = bot.supportFeature('stateIdUsed') &&
      window.type === 'minecraft:anvil' && !selectedItemBeforeClick && click.item
    let clickUpdate
    let clickUpdateHandled = false
    ...
    if (bot.supportFeature('stateIdUsed')) { // 1.17.1 +
      // Arm this after the local acceptClick prediction above. Otherwise the
      // prediction's synchronous updateSlot event would be mistaken for the
      // server response and the following transfer click could still race.
      if (!syncAfterPickup) {
        clickUpdate = waitForClickUpdate(window, slot, selectedItemBeforeClick)
      }
      ...
      if (syncAfterPickup) {
        // 26.2 may omit the response for a pickup from the player's
        // inventory. Send the authoritative sync after the pickup packet has
        // been queued; creating it before the write would reverse the order
        // and sync the pre-click state.
        clickUpdate = syncWindow(window).then(() => true)
      }
    ...
    if (bot.supportFeature('stateIdUsed')) {
      // State-id based servers process clicks asynchronously. Do not let a
      // transfer issue its destination click before the source click's local
      // prediction has been acknowledged; waiting on the destination/result
      // slot prevents a stale stateId from erasing the predicted result.
      clickUpdateHandled = await clickUpdate
    }
  ...
  async function putAway (slot) {
    const window = bot.currentWindow || bot.inventory
    if (window.type === 'minecraft:anvil' && slot === window.craftingResultSlot && !window.slots[slot]) {
      // The input-slot response and the computed anvil result can arrive as
      // separate packets. Do not click an empty local result slot while the
      // server's result is still in flight.
      await waitForAnvilResultAvailable(window, slot)
    }
    await clickWindow(slot, 0, 0)
  ...
  bot._client.on('set_cursor_item', (packet) => {
    const window = bot.currentWindow || bot.inventory
    if (!window) return
    window.selectedItem = Item.fromNotch(packet.contents)
    updateHeldItem()
    window.emit('setCursorItem')
  })
