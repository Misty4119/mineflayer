...
function inject (bot) {
  ...
  function teamHandler (packet) {
    ...
    const teamData = bot.supportFeature('teamPacketUsesOptionalColor')
      ? {
          name: packet.displayName,
          friendlyFire: packet.flags?.friendly_fire ?? false,
          nameTagVisibility: packet.nameTagVisibility,
          collisionRule: packet.collisionRule,
          formatting: packet.color ?? -1,
    ...
    switch (mode) {
      case 'add':
        team = new Team(
          ...
          teamData.nameTagVisibility,
          teamData.collisionRule,
          teamData.formatting,
          teamData.prefix,
          teamData.suffix
        )
