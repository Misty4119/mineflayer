...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  // Since 1.21.2 the vanilla client sends an empty tick_end packet after
  // every client tick. Modern servers use it to reset their per-tick
  // movement bookkeeping; without it, the second position packet is treated
  // as a duplicate and the client is kicked.
  const sendsClientTickEndPacket = bot.supportFeature('sendsClientTickEndPacket')
  ...
  function tickPhysics (now) {
    ...
    try {
      ...
      if (sendsClientTickEndPacket && bot._client.state === 'play') {
  ...
  function updatePosition (now) {
    // A 1.20.2+ client temporarily switches to CONFIGURATION while changing
    // backend servers. The protocol serializer follows that state immediately,
    // but the physics interval can still fire during the transition. Do not
    // serialize PLAY movement packets through the CONFIGURATION serializer.
  ...
  bot._client.on('position', (packet) => {
    ...
    if (bot.supportFeature('teleportUsesOwnPacket')) {
      const teleportConfirm = { teleportId: packet.teleportId }
      if (bot.version === '26.3') {
        teleportConfirm.x = packet.x
        teleportConfirm.y = packet.y
        teleportConfirm.z = packet.z
        teleportConfirm.yRot = packet.yaw
        teleportConfirm.xRot = packet.pitch
      }
      bot._client.write('teleport_confirm', teleportConfirm)
      if (bot.version === '26.3' && sendsClientTickEndPacket && bot._client.state === 'play') {
        // The 26.3 server counts teleport confirmation as movement. End the
        // tick immediately so the next movement packet is accepted.
        bot._client.write('tick_end', {})
      }
    }
    ...
    // In 26.3 the serverbound teleport confirmation carries the accepted
    // position and rotation. Sending a position packet in the same server
    // tick as that confirmation counts as a second movement packet and is
    // rejected before the first tick_end arrives.
    if (bot.version !== '26.3') {
