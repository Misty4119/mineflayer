...
function inject (bot) {
  ...
  bot._client.on('animation', (packet) => {
    // animation
    const entity = fetchEntity(packet.entityId)
    // 26.3 removed the old swing/hurt actions and renumbered the remaining
    // ClientboundAnimatePacket values.
    const eventName = (bot.version === '26.3' ? animationEvents26_3 : animationEvents)[packet.animation]
    if (eventName) bot.emit(eventName, entity)
  })

  bot._client.on('swing_animation', (packet) => {
    // 26.3 moved arm swings to ClientboundSwingAnimationPacket. Keep the
    // existing entitySwingArm event stable for plugins consuming it.
    bot.emit('entitySwingArm', fetchEntity(packet.entityId))
  })
  ...
  bot._client.on('rel_entity_move', (packet) => {
    ...
    const entity = fetchEntity(packet.entityId)
    if (bot.version === '26.3') {
      for (const step of packet.move.steps) {
        entity.position.translate(step.dX / (128 * 32), step.dY / (128 * 32), step.dZ / (128 * 32))
      }
      entity.onGround = packet.move.onGround
  ...
  bot._client.on('entity_look', (packet) => {
    ...
    entity.yaw = conv.fromNotchianYawByte(packet.yaw)
    entity.pitch = conv.fromNotchianPitchByte(packet.pitch)
    if (bot.version === '26.3') entity.onGround = packet.onGround
    bot.emit('entityMoved', entity)
  })
  ...
  bot._client.on('entity_move_look', (packet) => {
    ...
    entity.yaw = conv.fromNotchianYawByte(packet.yaw)
    entity.pitch = conv.fromNotchianPitchByte(packet.pitch)
    if (bot.version === '26.3') entity.onGround = packet.move.onGround
    bot.emit('entityMoved', entity)
  })
  ...
  bot._client.on('sync_entity_position', (packet) => {
    const entity = fetchEntity(packet.entityId)
    if (bot.version === '26.3') {
      const position = packet.positionType === 'stepped' ? packet.steps.at(-1) : packet
  ...
  function swingArm (arm = 'right', showHand = true) {
    ...
    if (bot.version === '26.3') {
      // ServerboundPunchPacket is empty in 26.3; the hand is no longer sent.
  ...
  function attack (target, swing = true) {
    // 26.1+: attack has its own packet and must be sent after the arm swing.
  ...
  function useEntity (target, leftClick, x, y, z) {
    ...
    if (bot.supportFeature('attackUsesOwnPacket')) {
      ...
      // 26.2 renamed this field; 26.1 still calls it sneaking.
      packet[bot.supportFeature('useEntityUsesSecondaryAction') ? 'usingSecondaryAction' : 'sneaking'] = sneaking
