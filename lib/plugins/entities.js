...
function inject (bot) {
  ...
  bot._client.on('animation', (packet) => {
    // animation
    const entity = fetchEntity(packet.entityId)
    // 26.3 removed the old swing animation and shifted the remaining
    // ClientboundAnimatePacket values down by one.
    const eventName = bot.version === '26.3' && packet.animation === 0
      ? 'entityWake'
      : animationEvents[packet.animation]
