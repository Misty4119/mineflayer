    ...
    static fromNetwork (packet) {
      if (registry.supportFeature('updatedParticlesPacket')) {
        ...
        return new Particle(
          ...
          packet.amount,
          // 26.3 carries an axis-specific maximum speed instead of the
          // legacy single velocityOffset field. The command protocol uses
          // the same value for all axes in the default randomization mode.
          packet.velocityOffset ?? packet.xMaxSpeed,
          packet.longDistance
        )
      } else {
        return new Particle(
